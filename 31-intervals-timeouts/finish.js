/** ЗАДАЧА 31 - Интервалы и Таймауты
 *
 * Используйте следующие встроенные функции:
 * setTimeout
 * setInterval
 * clearInterval
 *
 * Нужно вывести в консоль ТОЛЬКО 5 сообщений с интервалом 2 секунды:
 * "Сообщение номер 1"
 * "Сообщение номер 2"
 * "Сообщение номер 3"
 * "Сообщение номер 4"
 * "Сообщение номер 5"
 */

let i = 1

const myMessage = setInterval(() => {
    console.log(`Сообщение номер ${i}`)
    i += 1
}, 1000)

setTimeout(() => clearInterval(myMessage), 11000)

// повтор


let j = 1

const myMessageFunction = setInterval(() => {
    console.log(`Сообщение номер ${j}`)
    j += 1
}, 2000)

setTimeout(() => clearInterval(myMessageFunction), 11000)